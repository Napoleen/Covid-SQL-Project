...
--Regions by highest death count
Select location as Region , MAX(cast(Total_deaths as int)) as TotalDeathCount
From [Portfolio Project#1]..[Covid Deaths]
...
from PopvsVac
where TotalVaccinations is not null and TotalVaccinations >1 and TotalVaccinations is not null
order by VacPercentage asc,Location
...
--Create View for later data vis 
USE [Portfolio Project#1]
go 
Create view 
percentponvaccinated as 
Select dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations,
